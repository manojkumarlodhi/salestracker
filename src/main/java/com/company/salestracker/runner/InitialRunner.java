...
public class InitialRunner  implements CommandLineRunner{
	...
	public void run(String... args) throws Exception {
		...
		List<Permission>permissions=List.of(
			...
			Set<String>existingCodes=permissionRepo.findAll().stream().map(Permission::getPermissionCode).collect(Collectors.toSet());
			List<Permission>newPermission=permissions.stream()
					.filter(p -> !existingCodes.contains(p.getPermissionCode())).toList();
			if (!newPermission.isEmpty()) {
				permissionRepo.saveAll(newPermission);
			}
			...
			String email = "manojlodhi0262@gmail.com";

			if (userRepo.findByEmail(email).isEmpty()) {

				User superAdmin = User.builder()
						.name("manoj lodhi")
						.email(email).phone("7223938787")
